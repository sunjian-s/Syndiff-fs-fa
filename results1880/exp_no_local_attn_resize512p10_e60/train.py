...
class WaveletLoss(nn.Module):
    def __init__(self, device=None, high_weight=1.5):
        super(WaveletLoss, self).__init__()
        self.high_weight = high_weight
        self.register_buffer('to_gray_weight', torch.tensor([0.299, 0.587, 0.114], dtype=torch.float32).view(1, 3, 1, 1))
    ...
    def _to_grayscale(self, x):
        if x.shape[1] == 3:
            return (x * self.to_gray_weight).sum(dim=1, keepdim=True)
        if x.shape[1] == 1:
            return x
    ...
    def forward(self, input, target):
        input_gray = self._to_grayscale(input)
        target_gray = self._to_grayscale(target)

        input_ll, input_lh, input_hl, input_hh = self.haar_dwt(input_gray)
        target_ll, target_lh, target_hl, target_hh = self.haar_dwt(target_gray)

        loss_low = F.l1_loss(input_ll, target_ll)
        loss_high = (
            F.l1_loss(input_lh, target_lh)
            + F.l1_loss(input_hl, target_hl)
            + F.l1_loss(input_hh, target_hh)
        ) / 3.0

        return loss_low + self.high_weight * loss_high

# ============================================
# 【新增】2. 亮度加权损失 (LFSG的核心 - 亮度部分)
# ============================================
class LuminanceWeightedLoss(nn.Module):
    def __init__(self, weight_factor=3.0, dark_factor=1.0, focus_sharpness=2.0, device=None):
        """
        参数建议：
    ...
    def forward(self, pred, target):
        ...
        loss = torch.mean(diff * weight_map) / (weight_map.mean() + 1e-6)
...
def train_syndiff(rank, gpu, args):
    ...
    # Loss 实例化
    criterion_wavelet = WaveletLoss(device=device, high_weight=args.wavelet_high_weight).to(device)
    criterion_lumi = LuminanceWeightedLoss(
        weight_factor=args.lumi_bright_factor,
        dark_factor=args.lumi_dark_factor,
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in loader_iter:
            ...
            need_r1 = (args.lazy_reg is None) or (global_step % args.lazy_reg == 0)
            x1_t.requires_grad_(need_r1)
            x2_t.requires_grad_(need_r1)
            ...
            errD_real.backward(retain_graph=need_r1)
            ...
            # 总生成器损失
            errG = (
                args.lambda_cycle_l1 * errG_cycle
                + errG_adv
                + args.lambda_cycle_adv * errG_cycle_adv
                + args.lambda_pair_l1 * errG_L1
                + errG_innovative
            )
        ...
        if epoch % args.val_every == 0:
            ...
            # 5. 验证集循环 (Validation Loop)
            # -----------------------------------------------------------------
            # 方向 1: Domain 1 -> Domain 2 (确保通道切片逻辑与方向2一致)
            for iteration, (x_val , y_val) in enumerate(data_loader_val): 
                ...
                # 【补齐漏洞】添加生成张量的前3通道（RGB）提取
                fake_rgb = fake_sample1[:, :3, :, :]
                real_rgb = real_data[:, :3, :, :]

                # 修正：直接截断在 [0, 1] 物理有效域，严禁除以均值
                fake_rgb = torch.clamp(to_range_0_1(fake_rgb), 0.0, 1.0)
                real_rgb = torch.clamp(to_range_0_1(real_rgb), 0.0, 1.0)

                fake_rgb = fake_rgb.cpu().numpy()
                real_rgb = real_rgb.cpu().numpy()
                
                val_l1_loss[0, epoch, iteration] = abs(fake_rgb - real_rgb).mean()
                val_psnr_values[0, epoch, iteration] = psnr(real_rgb, fake_rgb, data_range=1.0) 
            ...
            for iteration, (y_val , x_val) in enumerate(data_loader_val):
                ...
                val_psnr_values[1, epoch, iteration] = psnr(real_rgb, fake_rgb, data_range=1.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_attn_resolutions', nargs='+', type=int, default=[64, 128],
                        help='resolutions where local attention is inserted in encoder and decoder')
    ...
    parser.add_argument('--viz_every', type=int, default=10, help='save sample visualizations every x epochs')
    ...
    parser.add_argument('--lambda_l1_loss', type=float, default=None, help='deprecated shared L1 weight for backward compatibility')
    parser.add_argument('--lambda_pair_l1', type=float, default=None, help='weight for paired diffusion reconstruction L1')
    parser.add_argument('--lambda_cycle_l1', type=float, default=None, help='weight for cycle reconstruction L1')
    parser.add_argument('--lambda_cycle_adv', type=float, default=0.2, help='weight for cycle adversarial loss')
    ...
    parser.add_argument('--lumi_bright_factor', type=float, default=3.0, help='bright-region weight factor for luminance loss')
    parser.add_argument('--lumi_dark_factor', type=float, default=1.0, help='dark-region weight factor for luminance loss')
    parser.add_argument('--lumi_focus_sharpness', type=float, default=2.0, help='focus sharpness for luminance loss weighting')
    parser.add_argument('--wavelet_high_weight', type=float, default=1.5, help='high-frequency weight for Haar wavelet loss')
    ...
    if args.lambda_pair_l1 is None and args.lambda_cycle_l1 is None:
        shared_l1 = 0.5 if args.lambda_l1_loss is None else args.lambda_l1_loss
        args.lambda_pair_l1 = shared_l1
        args.lambda_cycle_l1 = shared_l1
    else:
        if args.lambda_pair_l1 is None:
            args.lambda_pair_l1 = 0.5 if args.lambda_l1_loss is None else args.lambda_l1_loss
        if args.lambda_cycle_l1 is None:
            args.lambda_cycle_l1 = 0.25
