...
def process_single_pair_with_mask(netG, fundus_path, ffa_path, mask_path, args, device, pos_coeff, gaussian_weight):
    print(f"  处理: {os.path.basename(fundus_path)} ...", end="", flush=True)
    ...
    # 5. 滑动窗口生成
    output_canvas = torch.zeros((1, 4, h_orig, w_orig), device=device) # 注意这里也是4通道
...
def main():
    ...
    parser.add_argument('--num_channels', type=int, default=4) # 关键：这里是4
    ...
    parser.add_argument('--mask_dir', type=str, required=True, help='Mask 文件夹路径')
    parser.add_argument('--output_path', type=str, default='./inference_mask_results')
    ...
    print(f"加载模型 (4通道): {args.model_path}")
    ...
    for f in files:
        ...
        # 稍微宽容一点的 mask 匹配 (比如 1.jpg 对应 1.png 或 1_mask.png)
        ...
        comp_img, metrics = process_single_pair_with_mask(netG, fundus_path, ffa_path, mask_path, args, device, pos_coeff, gaussian_weight)
