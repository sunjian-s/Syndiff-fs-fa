...
class WaveletLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # 4. 【核心纠正】权重倒置！
        # 低频已经在全局 L1 和 Cycle 中被约束，这里降权到 0.5 甚至 0.1
        # 高频数值天然极小，必须赋予极高权重 (例如 5.0 或 10.0)，强迫网络画出锐利边缘
        weight_low = 0.5
        weight_high = 10.0 
...
#         return loss_low + 0.1 * loss_high # 让模型更关注高频细节

# ============================================
# 【新增】2. 亮度加权损失 (LFSG的核心 - 亮度部分)
# ============================================
class LuminanceWeightedLoss(nn.Module):
    """保留你的类名，但内部逻辑换成高亮极值惩罚"""
    def __init__(self, alpha=10.0, device='cuda'):
        super().__init__()
        self.alpha = alpha

    def forward(self, pred, target):
        # target_norm 范围拉到 [0, 1]，0是纯黑，1是极白微动脉瘤
        target_norm = (target + 1.0) * 0.5 
        # 极其暴力的放大器：白色区域权重是 1+10=11，灰色/黑色区域权重逼近 1
...
def train_syndiff(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        # 1. 基础的 Cycle 依然保留 (维持整体拓扑不崩)
            ...
            # (注意：errG_adv, errG_cycle_adv 应该在更上面的代码已经算好了)
            
            # 【致命逻辑修正】：你上面算好的 errG_L1 包含了 FA 图的普通 L1，它会掩盖病灶！
            # 我们在这里手动算出单向的 FS 彩照域 L1，并在后面的总和中彻底抛弃旧的 errG_L1
            errG1_L1_FS = F.l1_loss(x1_0_predict_diff[:,0:C,:,:], real_data1)
            ...
            # 【极其残酷且真实的权重配置】
            # ==========================================
            lambda_cycle = args.lambda_l1_loss * 0.5  # 通常是 1.0
            lambda_L1_FS = args.lambda_l1_loss * 0.5  # FS域保持普通 L1 1.0
            
            # 创新 Loss 必须拆开给权重！
            lambda_lumi = 2.0      # 【核心】顶掉原来 FA 域的 L1，权重必须大，强迫网络画病灶！
            lambda_wavelet = 0.5   # 约束高频边缘
            lambda_vgg = 0.5       # 防糊神器，不要再用 0.02 了
            lambda_cycle_adv = 0.1 # 降低循环对抗压力

            # ==========================================
            # 4. 重新组合总损失 errG
            # ==========================================
            errG = lambda_cycle * errG_cycle + \
                   errG_adv + \
                   lambda_cycle_adv * errG_cycle_adv + \
                   lambda_L1_FS * errG1_L1_FS + \
                   lambda_lumi * errG2_lumi + \
                   lambda_wavelet * (errG1_wavelet + errG2_wavelet) + \
                   lambda_vgg * errG_vgg
            ...
            # 【修正】对齐拆分后的具体 Loss
            epoch_losses_raw["G_l1_fs"] += errG1_L1_FS.item()
            epoch_losses_raw["G_lumi"] += errG2_lumi.item()
            epoch_losses_raw["G_wavelet"] += (errG1_wavelet + errG2_wavelet).item()
            epoch_losses_raw["G_vgg"] += errG_vgg.item()

            # 👇 【新增】累加加权值 (Weighted)
            epoch_losses_weighted["cycle_w"] += (lambda_cycle * errG_cycle).item()
            epoch_losses_weighted["cycle_adv_w"] += (lambda_cycle_adv * errG_cycle_adv).item()
            
            # 【修正】对齐拆分后的具体 Loss 和新定义的 lambda 权重
            epoch_losses_weighted["l1_fs_w"] += (lambda_L1_FS * errG1_L1_FS).item()
            epoch_losses_weighted["lumi_w"] += (lambda_lumi * errG2_lumi).item()
            epoch_losses_weighted["wavelet_w"] += (lambda_wavelet * (errG1_wavelet + errG2_wavelet)).item()
            ...
            if rank == 0:
                ...
                # 记录乘了系数后的创新损失实际大小
                # 记录乘了系数后的创新损失实际大小 (拆分后重新加和)
                current_inno_loss = (lambda_lumi * errG2_lumi).item() + \
                                    (lambda_wavelet * (errG1_wavelet + errG2_wavelet)).item()
