...
class LuminanceWeightedLoss(nn.Module):
    def __init__(self, weight_factor=10.0, dark_factor=10.0, focus_sharpness=2.0):
    ...
    def forward(self, pred, target):
        ...
        # 2. 计算双向权重
        bright_part = target_norm ** self.power
        dark_part = (1.0 - target_norm) ** self.power
...
def train_syndiff(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in loader_iter:
            ...
            # ✅【应用】创新 Loss
            errG1_lumi = criterion_lumi(x1_0_predict_diff[:,0:C,:], real_data1)* 0.0
            errG2_lumi = criterion_lumi(x2_0_predict_diff[:,0:C,:], real_data2)* 0.0
            errG1_wavelet = criterion_wavelet(x1_0_predict_diff[:,0:C,:], real_data1)* 0.0
            errG2_wavelet = criterion_wavelet(x2_0_predict_diff[:,0:C,:], real_data2)* 0.0
